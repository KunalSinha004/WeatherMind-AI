...
exports.requestAIPrediction = requestAIPrediction;
exports.generateFallbackPrediction = generateFallbackPrediction;
...
async function requestAIPrediction(payload) {
    ...
    try {
        ...
        const response = await axios_1.default.post(`${env_1.ENV.FASTAPI_AI_URL}${path}`, {
            location,
        ...
        console.warn(`[AI Proxy Service] FastAPI Python service offline/unreachable at ${env_1.ENV.FASTAPI_AI_URL}. Running local AI algorithm calculation fallback.`);
        return generateFallbackPrediction(location, metric, horizonDays, modelName, payload.currentVal);
    }
}
function generateFallbackPrediction(location, metric, horizonDays, modelName, currentVal) {
    const now = new Date();
